...
the key must be retained in the data store. Once the Time-To-Live for a key has expired, the key will no longer be available for Read or Delete operations. 7. Appropriate error responses must always be returned to a client if it uses the data store in
...
2. More than one client process cannot be allowed to use the same file as a data store any

given time

3. A client process is allowed to access the data store using multiple threads, if it desires The data store must therefore be thread-safe
